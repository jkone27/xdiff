...
let internal addNode parent child =
    match parent with
    | Node(name, attributes, childs) -> Node(name, attributes, child :: childs)
    | Leaf(_) -> failwith "parent is a leaf, cannot add child to a leaf"
    | Empty -> child

let internal GetAttributes (xel: XElement) =
    xel.Attributes() |> Seq.map (fun a -> a.Name.LocalName, a.Value) |> Map.ofSeq
...
let rec internal xelToDomain (x: XElement) (accumulator: XmlNode) : XmlNode =
    ...
    | false -> addNode accumulator (Leaf(name, GetAttributes x, x.Value))
...
let internal findDifferentChilds childsA childsB =
    ...
    let exB = ((childNamesB, childNamesA) ||> Set.difference) in
    (exA, exB)
